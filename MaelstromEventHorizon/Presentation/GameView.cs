...
internal sealed class GameView : FrameworkElement
{
    ...
    internal static readonly (TickerIcon Icon, string Name, string Description, uint Tint)[] TitleItemGuide = [(TickerIcon.Canister, "ITEM CANISTER", "contains one random ship power-up", 0xff50eaff), (TickerIcon.RapidFire, "RAPID FIRE", "hold fire for 24-round bursts", 0xffff6e72), (TickerIcon.AirBrakes, "AIR BRAKES", "adds strong momentum control", 0xff73d8ff), (TickerIcon.Luck, "LUCK OF THE IRISH", "triggers every special event once this wave", 0xff72f09a), (TickerIcon.TripleFire, "TRIPLE FIRE", "launches a three-way shot", 0xffffb85d), (TickerIcon.RiftVolley, "RIFT VOLLEY", "shots echo from twin delayed rifts", 0xffa774ff), (TickerIcon.LongRange, "LONG RANGE", "keeps shots active farther", 0xff8fd5ff), (TickerIcon.Shield, "SHIELD ENERGY", "restores shield charge", 0xff64edff), (TickerIcon.ReflectionShield, "REFLECTION SHIELD", "bounces one enemy shot back, then breaks", 0xffffc65b), (TickerIcon.Freeze, "TIME FREEZE", "stops everything except your ship for eight seconds", 0xffb18cff), (TickerIcon.SmartBomb, "SMART BOMB", "fractures asteroids and clears nearby threats", 0xffff7b68), (TickerIcon.RicochetArena, "RICOCHET ARENA", "seals the screen and makes every moving object bounce", 0xff74f3c5), (TickerIcon.GiantShip, "GIANT SHIP", "doubles ship size and absorbs one lethal hit", 0xffffd85a), (TickerIcon.Cash, "CASH BONUS", "adds dollars to the current wave", 0xffffd66b), (TickerIcon.Multiplier, "MULTIPLIER", "multiplies banked comet cash", 0xffd08aff), (TickerIcon.Comet, "COMET", "shoot any part for $1,000 to $5,000", 0xff73e7ff), (TickerIcon.RescueShip, "RESCUE SHIP", "touch it to gain one extra life", 0xff79f3b2)];
    ...
    internal void DrawWaveCompletionFlyby(DrawingContext dc) => Renderers.PlayerAsteroidRenderer.DrawWaveCompletionFlyby(this, dc);
