...
internal sealed class HazardPickupRenderer
{
    internal void DrawMines(GameView view, DrawingContext dc)
    {
        foreach (HomingMine mine in view.Game.Mines)
        {
            ...
            dc.DrawEllipse(
                new SolidColorBrush(Color.FromRgb(255, 55, 45)), null, new Point(),
    ...
    internal void DrawMineBody(GameView view, DrawingContext dc)
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            Geometry spikeShape = view.Polygon(
                (Math.Cos(a - .13) * 8, Math.Sin(a - .13) * 8), (Math.Cos(a) * 22, Math.Sin(a) * 22),
    ...
    internal void DrawVortices(GameView view, DrawingContext dc)
    {
        foreach (GravityVortex vortex in view.Game.Vortices)
        {
            ...
            dc.DrawEllipse(
                null, view.Pen(Color.FromArgb(48, 165, 103, 255), 1.2), view.Pt(vortex.Position), distortion,
                distortion * .64);
    ...
    internal void DrawVortexCore(DrawingContext dc)
    {
        ...
        dc.DrawEllipse(
            Brushes.Black, new Pen(new SolidColorBrush(Color.FromRgb(206, 164, 255)), 1), new Point(), 13, 13);

        dc.DrawEllipse(
            new RadialGradientBrush(Color.FromArgb(0, 255, 255, 255), Color.FromArgb(150, 191, 124, 255)), null,
            new Point(-7, -8), 18, 13);
    }
    ...
    internal void DrawNovas(GameView view, DrawingContext dc)
    {
        foreach (Nova nova in view.Game.Novas)
        {
            ...
            dc.DrawEllipse(
                null, view.Pen(Color.FromArgb((byte)(42 + progress * 85), 255, 164, 67), 1.1), view.Pt(nova.Position),
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                dc.DrawLine(
                    pen, view.Pt(nova.Position + V2.FromAngle(a) * radius * .55),
    ...
    internal void DrawPickups(GameView view, DrawingContext dc)
    {
        foreach (Pickup pickup in view.Game.Pickups)
        {
            ...
            view.DrawGlowEllipse(
                dc, pickup.Position, pickup.Radius * pulse, color, urgentPickup ? 9 : 7, urgentPickup ? .8 : .55);
            ...
            if (pickup.Kind is PickupKind.TimeFreeze or PickupKind.SmartBomb or PickupKind.RicochetArena)
            {
                ...
                dc.DrawEllipse(
                    null, view.Pen(Color.FromArgb(180, color.R, color.G, color.B), 1.5), view.Pt(pickup.Position),
                    beaconRadius * 2, beaconRadius * 2);

                dc.DrawArc(
                    view.Pen(Color.FromArgb(230, 255, 255, 255), 2.2), view.Pt(pickup.Position), beaconRadius + 4,
            ...
            if (pickup.Kind == PickupKind.Canister)
            {
                ...
                else
                {
                    ...
                    dc.DrawRoundedRectangle(
                        shell, new Pen(new SolidColorBrush(color), 1.5), new Rect(-11, -16, 22, 32), 5, 5);
            ...
            else if (pickup.Kind == PickupKind.RescueShip)
            {
                ...
                dc.DrawImage(view.RescueShipSprite, new Rect(
                    -hullCanvas / 2, -hullCanvas / 2, hullCanvas, hullCanvas));

                double navPulse = .72 + Math.Sin(view.Game.TotalTime * 8.5) * .18;

                dc.DrawEllipse(
                    new SolidColorBrush(Color.FromArgb((byte)(210 * navPulse), 94, 255, 177)),
            ...
            else if (pickup.Kind == PickupKind.TimeFreeze)
            {
                ...
                dc.DrawGeometry(
                    new SolidColorBrush(Color.FromArgb(210, 225, 255, 255)),
            ...
            else if (pickup.Kind == PickupKind.RicochetArena)
            {
                ...
                dc.DrawGeometry(
                    new SolidColorBrush(Color.FromArgb(125, color.R, color.G, color.B)),
                    new Pen(new SolidColorBrush(view.Lighten(color, .45)), 2.5), arena);

                dc.DrawGeometry(
                    new SolidColorBrush(Color.FromArgb(175, 20, 28, 58)),
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    dc.DrawEllipse(
                        new SolidColorBrush(Color.FromRgb(255, 244, 195)), new Pen(new SolidColorBrush(color), 1),
                        new Point(node.X, node.Y), 3.4, 3.4);
                }
            ...
            else if (pickup.Kind == PickupKind.SmartBomb)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    Geometry finShape = view.Polygon(
                        (fin.X * 9, fin.Y * 9), (fin.X * 21 - fin.Y * 4, fin.Y * 21 + fin.X * 4),
                        (fin.X * 16 + fin.Y * 4, fin.Y * 16 - fin.X * 4));

                    dc.DrawGeometry(
                        new SolidColorBrush(view.Darken(color, .5)),
                        new Pen(new SolidColorBrush(view.Lighten(color, .28)), 1), finShape);
                }

                dc.DrawEllipse(
                    new RadialGradientBrush(Color.FromRgb(255, 247, 185), Color.FromRgb(112, 15, 23)),
                    new Pen(new SolidColorBrush(view.Lighten(color, .42)), 2.5), new Point(), 16, 16);

                dc.DrawEllipse(
                    new SolidColorBrush(Color.FromArgb(170, 35, 0, 10)), new Pen(Brushes.White, 1), new Point(), 8,
                    8);

                dc.DrawLine(
                    new Pen(new SolidColorBrush(Color.FromRgb(255, 239, 140)), 2.5), new Point(5, -13),
                    new Point(12, -21));

                dc.DrawEllipse(
                    new SolidColorBrush(Color.FromRgb(255, 247, 112)), new Pen(Brushes.White, 1), new Point(13, -22),
                    4, 4);
            ...
            {
                BitmapImage? sprite = pickup.Kind == PickupKind.Multiplier
                    ? view.MultiplierSprite
                    : view.DollarSprite;
                ...
                else
                {
                    ...
                    dc.DrawGeometry(
                        new SolidColorBrush(view.Darken(color, .6)),
                ...
                view.DrawCenteredText(
                    dc, value, 1, 7.5, 11, new SolidColorBrush(Color.FromArgb(210, 0, 0, 0)), FontWeights.Black);
            ...
            for (int x = -copyRadius; x <= copyRadius; x++)
            {
                for (int y = -copyRadius; y <= copyRadius; y++)
                {
                    ...
                    if (position.X < -Comet.TrailLength - 30 ||
                        ...
                        position.Y < -Comet.TrailLength - 30 || position.Y > GameEngine.Height +
                                                                             Comet.TrailLength +
                                                                             30)
                    {
                        continue;
    ...
    private void DrawComet(GameView view, DrawingContext dc, Comet comet, V2 position)
    {
        ...
        view.DrawCenteredText(
            dc, comet.Value >= 1000 ? $"${comet.Value / 1000.0:0.#}K" : $"${comet.Value}", position.X, position.Y + 4,
            8.5, new SolidColorBrush(Color.FromRgb(30, 22, 28)), FontWeights.Black);
    }
