...
internal sealed class PlayerAsteroidRenderer
{
    ...
    internal void DrawShip(GameView view, DrawingContext dc)
    {
        ...
        V2 renderPosition = ship.Position;
        double renderAngle = ship.Angle;

        dc.PushTransform(new TranslateTransform(renderPosition.X, renderPosition.Y));
        dc.PushTransform(new RotateTransform(renderAngle * 180 / Math.PI));
