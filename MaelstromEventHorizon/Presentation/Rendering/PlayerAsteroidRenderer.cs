...
internal sealed class PlayerAsteroidRenderer
{
    internal void DrawWaveCompletionFlyby(GameView view, DrawingContext dc)
    {
        ...
        double progress = Math.Clamp(view.Game.TransitionElapsed / GameEngine.FadeToSummaryDuration, 0, 1);
        double scale = .38 + progress * 1.35;
        double opacity = .18 + progress * .22;
        ...
        LinearGradientBrush trail = new(Color.FromArgb(0, 60, 210, 255), Color.FromArgb(168, 220, 250, 255),
        ...
        Pen rim = new(new SolidColorBrush(Color.FromArgb(120, trailColor.R, trailColor.G, trailColor.B)),
            Math.Max(1, scale * .55));
    ...
    internal void DrawShip(GameView view, DrawingContext dc)
    {
        ...
        if (view.Game.PlayerRespawning)
        {
            return;
        }

        if (view.Game.Mode is GameMode.Title or GameMode.Controls or GameMode.Paused or GameMode.WaveSummary
            or GameMode.WaveSummaryExit)
        {
            return;
        }

        if (view.Game is { Lives: <= 0, Mode: GameMode.GameOverDelay or GameMode.NameEntry or GameMode.GameOver })
        {
            return;
        }
        ...
        if (ship.Shielding || ship.SpawnShieldTime > 0)
        {
            ...
            view.DrawGlowEllipse(dc, ship.Position, 29 * ship.VisualScale + pulse, shieldColor, 4,
                spawnShield ? .72 : .55);
            Pen shieldPen = new(new SolidColorBrush(Color.FromArgb(220, shieldColor.R, shieldColor.G, shieldColor.B)),
                1.9);
            dc.DrawEllipse(null, shieldPen, view.Pt(ship.Position), 29 * ship.VisualScale + pulse,
                29 * ship.VisualScale + pulse);
    ...
    internal void DrawDetailedShipHull(GameView view, DrawingContext dc, double time, Color accent, bool armored)
    {
        ...
        Pen seamPen = new(new SolidColorBrush(Color.FromArgb(165, 12, 25, 34)), .7);
    ...
    {
        foreach (ShipDebris piece in view.Game.ShipDebrisPieces)
        {
        ...
        if (rock.Steel)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    dc.DrawEllipse(new SolidColorBrush(Color.FromRgb(180, 234, 247)), new Pen(Brushes.Black, 1), p, 2.8,
                        2.8);
                }
            }
        }
    ...
    private void DrawRockTexture(GameView view, DrawingContext dc, Asteroid rock)
    {
        ...
        Pen vein = new(new SolidColorBrush(Color.FromArgb(100, 35, 27, 31)), Math.Max(.7, rock.Radius * .022));
