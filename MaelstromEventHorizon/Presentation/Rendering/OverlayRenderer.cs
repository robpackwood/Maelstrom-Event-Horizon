...
internal sealed class OverlayRenderer
{
    internal void DrawOverlay(GameView view, DrawingContext dc)
    {
        ...
        dc.DrawRectangle(
            new SolidColorBrush(
                Color.FromArgb(view.Game.Mode is GameMode.Title or GameMode.Controls ? (byte)92 : (byte)145, 0, 2, 9)),
        ...
        if (view.Game.Mode == GameMode.Title)
        {
            ...
            for (int i = 0; i < menuItems.Length; i++)
            {
                ...
                view.DrawText(dc, menuItems[i], 140, baseline, 21, brush,
        ...
        else if (view.Game.Mode == GameMode.NameEntry)
        {
            ...
            view.DrawCenteredText(dc, view.Game.PendingName + cursor, GameEngine.Width / 2, 404, 25, Brushes.White,
                FontWeights.Bold);
        ...
        else
        {
            ...
            view.DrawCenteredText(dc, $"FINAL SCORE  {view.Money(view.Game.Score)}     WAVE {view.Game.Wave}",
                GameEngine.Width / 2, 139, 18, Brushes.White, FontWeights.Bold);
            ...
            view.DrawCenteredText(dc, "PRESS ENTER FOR TITLE", GameEngine.Width / 2, 626, 15, Brushes.White,
                FontWeights.Bold);
        }
    ...
    private void DrawGameOverScene(GameView view, DrawingContext dc, bool highScore)
    {
        ...
        for (int i = 0; i < 22; i++)
        {
            ...
            dc.DrawGeometry(new SolidColorBrush(Color.FromArgb(170, metal.R, metal.G, metal.B)),
                ...
                view.Polygon((-size, -size * .35), (size * .7, -size * .55), (size, size * .2),
                    (-size * .4, size * .65)));
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            dc.DrawLine(sparkPen, spark,
                new Point(spark.X - Math.Cos(phase) * length, spark.Y - Math.Sin(phase) * length));
        }
    }
    ...
    private void DrawWaveSummary(GameView view, DrawingContext dc)
    {
        ...
        dc.DrawLine(new Pen(new SolidColorBrush(Color.FromArgb(135, 77, 192, 220)), 1), new Point(390, 181),
            new Point(890, 181));
        ...
        view.DrawText(dc, view.Game.IsBonusStage ? "DODGE EARNINGS" : "WAVE EARNINGS", 420, 222, 15, label,
            FontWeights.SemiBold);
        ...
        dc.DrawLine(new Pen(new SolidColorBrush(Color.FromArgb(135, 77, 192, 220)), 1), new Point(410, 405),
            new Point(870, 405));
    ...
    {
        view.DrawText(dc, "TOP 10 PILOTS", 704, 61, 21, new SolidColorBrush(Color.FromRgb(145, 220, 241)),
            FontWeights.Bold);
        ...
        dc.DrawLine(new Pen(new SolidColorBrush(Color.FromArgb(95, 85, 160, 190)), 1), new Point(700, 108),
            new Point(1194, 108));
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            {
                view.DrawText(dc, "---", 758, baseline, 14, new SolidColorBrush(Color.FromRgb(65, 85, 99)),
                    FontWeights.Normal);
            }
        }
    }
