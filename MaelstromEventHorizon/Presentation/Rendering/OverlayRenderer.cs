...
internal sealed class OverlayRenderer
{
    ...
    {
        if (view.Game.Mode is GameMode.Playing or GameMode.WaveOutro or GameMode.WaveIntro or GameMode.GameOverDelay)
        {
            return;
        }
        ...
        dc.DrawRectangle(
            new SolidColorBrush(
                Color.FromArgb(view.Game.Mode is GameMode.Title or GameMode.Controls
                    ? (byte)92
                    : (byte)145, 0, 2, 9)),
        ...
        {
            view.DrawText(
                dc, "MAELSTROM", 82, 72, 58, new SolidColorBrush(Color.FromRgb(225, 247, 255)), FontWeights.Black);

            view.DrawText(
                dc, "EVENT HORIZON", 88, 111, 19, new SolidColorBrush(Color.FromRgb(82, 221, 255)),
                FontWeights.SemiBold);

            dc.DrawLine(
                new Pen(new SolidColorBrush(Color.FromArgb(150, 84, 219, 255)), 1), new Point(88, 132),
                new Point(551, 132));
            ...
            for (int i = 0; i < menuItems.Length; i++)
            {
                ...
                Brush brush =
                    new SolidColorBrush(selected ? Color.FromRgb(117, 230, 255) : Color.FromRgb(174, 194, 207));

                if (selected)
                {
                    view.DrawText(dc, ">", 101, baseline, 21, brush, FontWeights.Bold);
                }

                view.DrawText(
                    dc, menuItems[i], 140, baseline, 21, brush, selected ? FontWeights.Bold : FontWeights.SemiBold);

                if (i is 2 or 3)
                {
                    double volume = i == 2 ? view.Game.MusicVolume : view.Game.EffectsVolume;
            ...
            dc.DrawLine(
                new Pen(new SolidColorBrush(Color.FromArgb(100, 84, 155, 184)), 1), new Point(642, 38),
                new Point(642, 542));
        ...
        else if (view.Game.Mode == GameMode.QuitConfirm)
        {
            dc.DrawRoundedRectangle(
                new SolidColorBrush(Color.FromArgb(235, 4, 12, 22)),
                new Pen(new SolidColorBrush(Color.FromRgb(104, 207, 235)), 1.4), new Rect(382, 243, 516, 224), 5, 5);

            view.DrawCenteredText(
                dc, "RETURN TO TITLE?", GameEngine.Width / 2, 305, 32,
                new SolidColorBrush(Color.FromRgb(230, 247, 252)), FontWeights.Black);

            view.DrawCenteredText(
                dc, "THE CURRENT RUN WILL END", GameEngine.Width / 2, 356, 14,
                new SolidColorBrush(Color.FromRgb(255, 180, 128)), FontWeights.SemiBold);

            view.DrawCenteredText(
                dc, "ENTER  CONTINUE", 524, 421, 14, new SolidColorBrush(Color.FromRgb(118, 239, 168)),
                FontWeights.Bold);

            view.DrawCenteredText(
                dc, "ESC  EXIT", 756, 421, 14, new SolidColorBrush(Color.FromRgb(128, 213, 241)), FontWeights.Bold);
        }
        else if (view.Game.Mode == GameMode.ClearHighScoresConfirm)
        {
            ...
            view.DrawCenteredText(
                dc, "CLEAR HIGH SCORES?", GameEngine.Width / 2, 305, 32,
                new SolidColorBrush(Color.FromRgb(230, 247, 252)), FontWeights.Black);

            view.DrawCenteredText(
                dc, "THIS CANNOT BE UNDONE", GameEngine.Width / 2, 356, 14,
                new SolidColorBrush(Color.FromRgb(255, 180, 128)), FontWeights.SemiBold);

            view.DrawCenteredText(
                dc, "ENTER  CLEAR", 524, 421, 14, new SolidColorBrush(Color.FromRgb(255, 180, 128)),
                FontWeights.Bold);

            view.DrawCenteredText(
                dc, "ESC  CANCEL", 756, 421, 14, new SolidColorBrush(Color.FromRgb(128, 213, 241)), FontWeights.Bold);
        }
        ...
        else if (view.Game.Mode == GameMode.NameEntry)
        {
            ...
            view.DrawCenteredText(
                dc, "GAME OVER", GameEngine.Width / 2, 105, 50, new SolidColorBrush(Color.FromRgb(255, 105, 97)),
                FontWeights.Black);

            view.DrawCenteredText(
                dc, "TOP 10 SCORE", GameEngine.Width / 2, 190, 38, new SolidColorBrush(Color.FromRgb(109, 224, 255)),
                FontWeights.Black);

            view.DrawCenteredText(
                dc,
                $"RANK {view.Game.PendingHighScoreRank:00}  /  {view.Money(view.Game.Score)}  /  WAVE {view.Game.Wave}",
                GameEngine.Width / 2, 236, 18, Brushes.White, FontWeights.Bold);

            view.DrawCenteredText(
                dc, "ENTER PILOT NAME", GameEngine.Width / 2, 342, 15,
                new SolidColorBrush(Color.FromRgb(151, 187, 207)), FontWeights.SemiBold);

            dc.DrawRoundedRectangle(
                new SolidColorBrush(Color.FromArgb(210, 4, 13, 24)),
            ...
            view.DrawCenteredText(
                dc, view.Game.PendingName + cursor, GameEngine.Width / 2, 404, 25, Brushes.White, FontWeights.Bold);

            view.DrawCenteredText(
                dc, "ENTER TO SAVE", GameEngine.Width / 2, 469, 13, new SolidColorBrush(Color.FromRgb(122, 215, 239)),
                FontWeights.SemiBold);
        }
        else
        {
            ...
            view.DrawCenteredText(
                dc, "GAME OVER", GameEngine.Width / 2, 92, 48, new SolidColorBrush(Color.FromRgb(255, 105, 97)),
                FontWeights.Black);

            view.DrawCenteredText(
                dc, $"FINAL SCORE  {view.Money(view.Game.Score)}     WAVE {view.Game.Wave}", GameEngine.Width / 2, 139,
                18, Brushes.White, FontWeights.Bold);

            view.DrawCenteredText(
                dc, "TOP 10 PILOTS", GameEngine.Width / 2, 190, 20, new SolidColorBrush(Color.FromRgb(143, 194, 222)),
                FontWeights.SemiBold);

            view.DrawHighScores(dc);

            view.DrawCenteredText(
                dc, "PRESS ENTER FOR TITLE", GameEngine.Width / 2, 626, 15, Brushes.White, FontWeights.Bold);
        }

        dc.Pop();
    }

    private static void DrawOptionValue(
        GameView view, DrawingContext dc, string value, double x, double baseline, bool selected)
    {
        Brush valueBrush = new SolidColorBrush(selected
            ? Color.FromRgb(255, 222, 110)
            : Color.FromRgb(137, 178, 195));

        view.DrawText(dc, $"<  {value}  >", x, baseline, 15, valueBrush, FontWeights.Bold);
    }

    private void DrawVolumeSlider(
        GameView view, DrawingContext dc, double x, double y, double width, double value, bool selected)
    {
        value = Math.Clamp(value, 0, 1);
        Color accent = selected ? Color.FromRgb(117, 230, 255) : Color.FromRgb(100, 160, 183);
        Rect track = new(x, y + 4, width, 7);

        dc.DrawRoundedRectangle(
            new SolidColorBrush(Color.FromArgb(205, 3, 14, 25)),
            new Pen(new SolidColorBrush(Color.FromArgb(145, 111, 161, 181)), 1), track, 3, 3);

        if (value > 0)
        {
            dc.DrawRoundedRectangle(
                new SolidColorBrush(Color.FromArgb(230, accent.R, accent.G, accent.B)), null,
        ...
        for (int i = 0; i <= 4; i++)
        {
            ...
            dc.DrawLine(
                new Pen(new SolidColorBrush(Color.FromArgb(125, 183, 220, 231)), .7), new Point(tickX, y + 2),
                new Point(tickX, y + 13));
        }

        double knobX = x + width * value;

        dc.DrawEllipse(
            new SolidColorBrush(selected ? Color.FromRgb(255, 225, 121) : Color.FromRgb(203, 226, 231)),
            new Pen(new SolidColorBrush(Color.FromRgb(16, 47, 60)), 1.2), new Point(knobX, y + 7.5), 6, 6);

        view.DrawText(
            dc, $"{Math.Round(value * 100):0}%", x + width + 15, y + 13, 12,
            new SolidColorBrush(selected ? Color.FromRgb(255, 225, 121) : Color.FromRgb(164, 194, 205)),
            FontWeights.SemiBold);
    }
    ...
    private void DrawGameOverScene(GameView view, DrawingContext dc, bool highScore)
    {
        ...
        {
            dc.DrawLine(
                new Pen(new SolidColorBrush(Color.FromArgb(10, 124, 213, 234)), .6), new Point(30, y),
                new Point(GameEngine.Width - 30, y));
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Pen orbitPen = new(new SolidColorBrush(Color.FromArgb(alpha, 89, 212, 242)), 1);
        ...
        for (int i = 0; i < 22; i++)
        {
            ...
            dc.DrawGeometry(new SolidColorBrush(Color.FromArgb(170, metal.R, metal.G, metal.B)),
                ...
                view.Polygon(
                    (-size, -size * .35), (size * .7, -size * .55), (size, size * .2), (-size * .4, size * .65)));
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            Pen sparkPen = new(new SolidColorBrush(Color.FromArgb(155, 255, 167, 77)), 1.2);

            dc.DrawLine(
                sparkPen, spark, new Point(spark.X - Math.Cos(phase) * length, spark.Y - Math.Sin(phase) * length));
        }
    }
    ...
    private void DrawWaveSummary(GameView view, DrawingContext dc)
    {
        ...
        dc.DrawLine(
            new Pen(new SolidColorBrush(Color.FromArgb(135, 77, 192, 220)), 1), new Point(390, 181),
            new Point(890, 181));

        if (view.Game.BonusStageFailed)
        {
            view.DrawCenteredText(
                dc, "ASTEROID COLLISION  /  NO BONUS AWARDED", GameEngine.Width / 2, 202, 11,
                new SolidColorBrush(Color.FromRgb(255, 166, 146)), FontWeights.Bold);
        }

        SolidColorBrush label = new(Color.FromRgb(139, 177, 197));
        SolidColorBrush value = new(Color.FromRgb(235, 247, 251));

        view.DrawText(
            dc, view.Game.IsBonusStage ? "DODGE EARNINGS" : "WAVE EARNINGS", 420, 222, 15, label,
            FontWeights.SemiBold);
        ...
        view.DrawText(
            dc, view.Money(view.Game.SummaryLevelBonusCash), 733, 261, 18,
        ...
        view.DrawText(
            dc, $"x {view.Game.SummaryMultiplier}", 733, 339, 18, new SolidColorBrush(Color.FromRgb(217, 159, 255)),
            FontWeights.Bold);
        ...
        view.DrawText(
            dc,
            view.Money((view.Game.SummaryLevelBonusCash + view.Game.SummaryCometCash) * view.Game.SummaryMultiplier),
            733, 378, 18, new SolidColorBrush(Color.FromRgb(123, 229, 255)), FontWeights.Bold);

        dc.DrawLine(
            new Pen(new SolidColorBrush(Color.FromArgb(135, 77, 192, 220)), 1), new Point(410, 405),
            new Point(870, 405));

        view.DrawText(dc, "WAVE DEPOSIT", 420, 448, 17, Brushes.White, FontWeights.Bold);

        view.DrawText(
            dc, view.Money(view.Game.SummaryDeposited), 733, 448, 23,
            new SolidColorBrush(Color.FromRgb(114, 255, 157)), FontWeights.Black);

        view.DrawCenteredText(
            dc, $"BANK TOTAL  {view.Money(view.Game.Score)}", GameEngine.Width / 2, 516, 19,
        ...
        view.DrawCenteredText(
            dc, prompt, GameEngine.Width / 2, 608, 14, new SolidColorBrush(Color.FromRgb(139, 206, 226)),
            FontWeights.SemiBold);
    }

    internal void DrawTransitionCurtain(GameView view, DrawingContext dc)
    {
        if (view.Game.TransitionAlpha <= 0)
        {
            return;
        }
        ...
        dc.DrawRectangle(
            new SolidColorBrush(Color.FromArgb(alpha, 0, 1, 5)), null,
    ...
    {
        view.DrawText(
            dc, "TOP 10 PILOTS", 704, 61, 21, new SolidColorBrush(Color.FromRgb(145, 220, 241)), FontWeights.Bold);

        SolidColorBrush dim = new(Color.FromRgb(106, 140, 159));
        ...
        dc.DrawLine(
            new Pen(new SolidColorBrush(Color.FromArgb(95, 85, 160, 190)), 1), new Point(700, 108),
            new Point(1194, 108));
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            {
                dc.DrawRoundedRectangle(
                    new SolidColorBrush(Color.FromArgb(72, 255, 211, 83)),
            ...
            {
                view.DrawText(
                    dc, "---", 758, baseline, 14, new SolidColorBrush(Color.FromRgb(65, 85, 99)), FontWeights.Normal);
            }
        }
    }
}
