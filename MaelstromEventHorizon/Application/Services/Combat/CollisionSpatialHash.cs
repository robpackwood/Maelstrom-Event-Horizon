...
internal sealed class CollisionSpatialHash
{
    ...
    private readonly List<Body>[] cells = Enumerable.Range(0, Columns * Rows).Select(_ => new List<Body>(8)).ToArray();
    private readonly List<Body> nearby = new(32);

    internal void Build(GameEngine game)
    {
        foreach (List<Body> cell in cells) cell.Clear();
        Add(game.Asteroids); Add(game.Fighters); Add(game.Bosses); Add(game.Mines); Add(game.Vortices); Add(game.Novas);
    }
    ...
    private void Add<T>(List<T> bodies) where T : Body
    {
        ...
        {
            T body = bodies[i]; if (!body.Alive) continue;
            int x = Math.Clamp((int)(body.Position.X / CellSize), 0, Columns - 1);
            int y = Math.Clamp((int)(body.Position.Y / CellSize), 0, Rows - 1);
