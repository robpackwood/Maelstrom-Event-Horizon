...
{
    private static bool HasAlive<T>(List<T> bodies) where T : Body
    {
    ...
    internal void BeginNextWave(GameEngine game)
    {
        ...
        else if (!game.IsBonusStage && game.RestoreGiantShipAfterBonus)
        {
        ...
        if (game.IsBossStage)
        {
            game.Announce("WARNING! BOSS DETECTED", 3.2);
            game.Audio.Play(SoundCue.BossAlarm);
        }
        else
        {
            string banner = $"WAVE {game.Wave}";
    ...
    private void SpawnAlienBoss(GameEngine game)
    {
        ...
        AlienBossKind kind = (AlienBossKind)((encounter - 1) % 6);
    ...
    {
        if (game.BonusAsteroidsRemaining <= 0)
        {
            return;
        }
    ...
    private void SpawnSlalomGate(GameEngine game, int difficulty)
    {
        ...
        game.BonusAsteroidSpawnTimer = Math.Max(.82, 1.12 - difficulty * .035);
    }
    ...
    {
        if (HasAlive(game.Fighters))
        {
            return;
        }

        if (game.FighterSpawnedThisWave)
        {
            return;
        }
    ...
    {
        if (HasAlive(game.Fighters) || game.FighterSpawnedThisWave)
        {
            return;
        }
    ...
    internal void SpawnVortex(GameEngine game)
    {
        ...
        game.Announce("BLACK HOLE DETECTED", 1.6);
    ...
    internal void SpawnVortexAssault(GameEngine game)
    {
        ...
        game.Announce("BLACK HOLE DETECTED", 2.2);
        game.Audio.Play(SoundCue.BossAlarm);
        game.Audio.Play(SoundCue.Vortex, .78);
    }

    internal void SpawnNova(GameEngine game)
    {
        game.Novas.Add(new Nova(game.SafePosition(300)));
        game.Announce("SUPERNOVA DETECTED", 1.8);
    ...
    internal void SpawnNovaAssault(GameEngine game)
    {
        ...
        game.Announce("SUPERNOVA DETECTED", 2.2);
        game.Audio.Play(SoundCue.BossAlarm);
        game.Audio.Play(SoundCue.Nova, .7);
    }
    ...
    internal void SpawnComet(GameEngine game)
    {
        ...
        SpawnCometEntity(game);
        game.Announce("BONUS COMET", 1.25);
    }
    ...
    internal void SpawnRescueShip(GameEngine game)
    {
        ...
        {
            if (game.Pickups[i] is { Alive: true, Kind: PickupKind.RescueShip })
            {
                return;
            }
        }
